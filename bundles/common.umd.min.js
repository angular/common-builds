 ...
 */
function PopStateEvent(){}function _stripBaseHref(baseHref,url){return baseHref&&url.startsWith(baseHref)?url.substring(baseHref.length):url}function _stripIndexHtml(url){return url.replace(/\/index.html$/,"")}function getLocaleId(locale){return findLocaleData(locale)[LocaleDataIndex.LocaleId]}function getLocaleDayPeriods(locale,formStyle,width){var data=findLocaleData(locale),amPmData=[data[LocaleDataIndex.DayPeriodsFormat],data[LocaleDataIndex.DayPeriodsStandalone]],amPm=getLastDefinedValue(amPmData,formStyle);return getLastDefinedValue(amPm,width)}function getLocaleDayNames(locale,formStyle,width){var data=findLocaleData(locale),daysData=[data[LocaleDataIndex.DaysFormat],data[LocaleDataIndex.DaysStandalone]],days=getLastDefinedValue(daysData,formStyle);return getLastDefinedValue(days,width)}function getLocaleMonthNames(locale,formStyle,width){var data=findLocaleData(locale),monthsData=[data[LocaleDataIndex.MonthsFormat],data[LocaleDataIndex.MonthsStandalone]],months=getLastDefinedValue(monthsData,formStyle);return getLastDefinedValue(months,width)}function getLocaleEraNames(locale,width){var data=findLocaleData(locale),erasData=data[LocaleDataIndex.Eras];return getLastDefinedValue(erasData,width)}function getLocaleFirstDayOfWeek(locale){var data=findLocaleData(locale);return data[LocaleDataIndex.FirstDayOfWeek]}function getLocaleWeekEndRange(locale){var data=findLocaleData(locale);return data[LocaleDataIndex.WeekendRange]}function getLocaleDateFormat(locale,width){var data=findLocaleData(locale);return data[LocaleDataIndex.DateFormat][width]}function getLocaleTimeFormat(locale,width){var data=findLocaleData(locale);return data[LocaleDataIndex.TimeFormat][width]}function getLocaleDateTimeFormat(locale,width){var data=findLocaleData(locale),dateTimeFormatData=data[LocaleDataIndex.DateTimeFormat];return getLastDefinedValue(dateTimeFormatData,width)}function getLocaleNumberSymbol(locale,symbol){var data=findLocaleData(locale),res=data[LocaleDataIndex.NumberSymbols][symbol];if("undefined"==typeof res){if(symbol===NumberSymbol.CurrencyDecimal)return data[LocaleDataIndex.NumberSymbols][NumberSymbol.Decimal];if(symbol===NumberSymbol.CurrencyGroup)return data[LocaleDataIndex.NumberSymbols][NumberSymbol.Decimal]}return res}function getLocaleNumberFormat(locale,type){var data=findLocaleData(locale);return data[LocaleDataIndex.NumberFormats][type]}function getLocaleCurrencySymbol(locale){var data=findLocaleData(locale);return data[LocaleDataIndex.CurrencySymbol]||null}function getLocaleCurrencyName(locale){var data=findLocaleData(locale);return data[LocaleDataIndex.CurrencyName]||null}function getLocalePluralCase(locale){var data=findLocaleData(locale);return data[LocaleDataIndex.PluralCase]}function checkFullData(data){if(!data[LocaleDataIndex.ExtraData])throw new Error('Missing extra locale data for the locale "'+data[LocaleDataIndex.LocaleId]+'". Use "registerLocaleData" to load new data. See the "I18n guide" on angular.io to know more.')}function getLocaleExtraDayPeriodRules(locale){var data=findLocaleData(locale);checkFullData(data);var rules=data[LocaleDataIndex.ExtraData][ExtraLocaleDataIndex.ExtraDayPeriodsRules]||[];return rules.map(function(rule){return"string"==typeof rule?extractTime(rule):[extractTime(rule[0]),extractTime(rule[1])]})}function getLocaleExtraDayPeriods(locale,formStyle,width){var data=findLocaleData(locale);checkFullData(data);var dayPeriodsData=[data[LocaleDataIndex.ExtraData][ExtraLocaleDataIndex.ExtraDayPeriodFormats],data[LocaleDataIndex.ExtraData][ExtraLocaleDataIndex.ExtraDayPeriodStandalone]],dayPeriods=getLastDefinedValue(dayPeriodsData,formStyle)||[];return getLastDefinedValue(dayPeriods,width)||[]}function getLastDefinedValue(data,index){for(var i=index;i>-1;i--)if("undefined"!=typeof data[i])return data[i];throw new Error("Locale data API: locale data undefined")}function extractTime(time){var _a=time.split(":"),h=_a[0],m=_a[1];return{hours:+h,minutes:+m}}function findLocaleData(locale){var normalizedLocale=getNormalizedLocale(locale);if("en"===normalizedLocale)return LOCALE_DATA.en||localeEn;var match=LOCALE_DATA[toCamelCase(normalizedLocale)];if(match)return match;throw new Error('Missing locale data for the locale "'+locale+'". Use "registerLocaleData" to load new data. See the "I18n guide" on angular.io to know more.')}function getNormalizedLocale(locale){if(NORMALIZED_LOCALES[locale])return NORMALIZED_LOCALES[locale];var normalizedLocale=locale.toLowerCase().replace(/_/g,"-"),match=AVAILABLE_LOCALES.find(function(l){return l.toLowerCase()===normalizedLocale});if(match)return NORMALIZED_LOCALES[locale]=match,match;var parentLocale=normalizedLocale.split("-")[0];if(AVAILABLE_LOCALES.find(function(l){return l.toLowerCase()===parentLocale}))return NORMALIZED_LOCALES[locale]=parentLocale,parentLocale;throw new Error('"'+locale+'" is not a valid LOCALE_ID value. See https://github.com/unicode-cldr/cldr-core/blob/master/availableLocales.json for a list of valid locales')}function toCamelCase(str){return str.replace(/-+([a-z0-9A-Z])/g,function(){for(var m=[],_i=0;_i<arguments.length;_i++)m[_i]=arguments[_i];return m[1].toUpperCase()})}function findCurrencySymbol(code,format){var currency=CURRENCIES[code]||{},symbol=currency[0]||code;return"wide"===format?symbol:currency[1]||symbol}function registerLocaleData(data,extraData){var localeId=toCamelCase(data[LocaleDataIndex.LocaleId]);LOCALE_DATA[localeId]=data,extraData&&(LOCALE_DATA[localeId][LocaleDataIndex.ExtraData]=extraData)}function getPluralCategory(value,cases,ngLocalization,locale){var key="="+value;if(cases.indexOf(key)>-1)return key;if(key=ngLocalization.getPluralCategory(value,locale),cases.indexOf(key)>-1)return key;if(cases.indexOf("other")>-1)return"other";throw new Error('No plural message found for value "'+value+'"')}/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 ...
 * found in the LICENSE file at https://angular.io/license
 */
var _INTERPOLATION_REGEXP=/#/g,I18nPluralPipe=function(){function I18nPluralPipe(_localization){this._localization=_localization}return I18nPluralPipe.prototype.transform=function(value,pluralMap,locale){if(null==value)return"";if("object"!=typeof pluralMap||null===pluralMap)throw invalidPipeArgumentError(I18nPluralPipe,pluralMap);var key=getPluralCategory(value,Object.keys(pluralMap),this._localization,locale);return pluralMap[key].replace(_INTERPOLATION_REGEXP,value.toString())},I18nPluralPipe}();I18nPluralPipe.decorators=[{type:_angular_core.Pipe,args:[{name:"i18nPlural",pure:!0}]}],I18nPluralPipe.ctorParameters=function(){return[{type:NgLocalization}]};/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
 ...
 * found in the LICENSE file at https://angular.io/license
 */
var DecimalPipe=function(){function DecimalPipe(_locale){this._locale=_locale}return DecimalPipe.prototype.transform=function(value,digits,locale){if(isEmpty(value))return null;locale=locale||this._locale;var _a=formatNumber$1(value,locale,NumberFormatStyle.Decimal,digits),str=_a.str,error=_a.error;if(error)throw invalidPipeArgumentError(CurrencyPipe,error);return str},DecimalPipe}();DecimalPipe.decorators=[{type:_angular_core.Pipe,args:[{name:"number"}]}],DecimalPipe.ctorParameters=function(){return[{type:void 0,decorators:[{type:_angular_core.Inject,args:[_angular_core.LOCALE_ID]}]}]};var PercentPipe=function(){function PercentPipe(_locale){this._locale=_locale}return PercentPipe.prototype.transform=function(value,digits,locale){if(isEmpty(value))return null;locale=locale||this._locale;var _a=formatNumber$1(value,locale,NumberFormatStyle.Percent,digits),str=_a.str,error=_a.error;if(error)throw invalidPipeArgumentError(CurrencyPipe,error);return str},PercentPipe}();PercentPipe.decorators=[{type:_angular_core.Pipe,args:[{name:"percent"}]}],PercentPipe.ctorParameters=function(){return[{type:void 0,decorators:[{type:_angular_core.Inject,args:[_angular_core.LOCALE_ID]}]}]};var CurrencyPipe=function(){function CurrencyPipe(_locale){this._locale=_locale}return CurrencyPipe.prototype.transform=function(value,currencyCode,display,digits,locale){if(void 0===display&&(display="symbol"),isEmpty(value))return null;locale=locale||this._locale,"boolean"==typeof display&&(console&&console.warn&&console.warn('Warning: the currency pipe has been changed in Angular v5. The symbolDisplay option (third parameter) is now a string instead of a boolean. The accepted values are "code", "symbol" or "symbol-narrow".'),display=display?"symbol":"code");var currency=currencyCode||"USD";"code"!==display&&(currency=findCurrencySymbol(currency,"symbol"===display?"wide":"narrow"));var _a=formatNumber$1(value,locale,NumberFormatStyle.Currency,digits,currency),str=_a.str,error=_a.error;if(error)throw invalidPipeArgumentError(CurrencyPipe,error);return str},CurrencyPipe}();CurrencyPipe.decorators=[{type:_angular_core.Pipe,args:[{name:"currency"}]}],CurrencyPipe.ctorParameters=function(){return[{type:void 0,decorators:[{type:_angular_core.Inject,args:[_angular_core.LOCALE_ID]}]}]};/**
 * @license
 * Copyright Google Inc. All Rights Reserved.
 *
 * Use of this source code is governed by an MIT-style license that can be
